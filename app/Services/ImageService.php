    ...
    foreach ($images as $image) {
      ...
      foreach($image['imageConfigurations'] as $size => $configuration){
        ...
        foreach ($image['files'] as $file) {
          ...
          // $buffer = file_get_contents($disk->path($originalImagePath));

          // $resizedImage = Image::newFromBuffer($buffer)->thumbnail_image($configuration['widthPx'], [
          //   'height' => $configuration['heightPx'],
          //   'crop' => 'centre'
          // ])->writeToBuffer('.webp', [
          //   'Q' => 80
          // ]);

          // $disk->put($resizedImagePath, $resizedImage);

          // $imagesResized[$image['languageAlias']][$size][$image['name']][] = $file;
        }
      }
    }
