    ...
    <div class="table__body">
      ...
      <div class="table__body-box">
        <div class="table-box__data" data-id="{{ $record->id }}">
          <h3>{{ $record->fiscal_name }}</h3>
          <p><span>NIF:</span> {{ $record->nif }}</p>
          <p><span>Tax:</span> {{ $record->tax_amount }}</p>
          <p><span>Ticket:</span> {{ $record->ticket_number }}</p>
          <p><span>Fecha ticket:</span> {{ $record->datetime }}</p>
          <p><span>Fecha guardado:</span> {{ $record->created_at }}</p>
        </div>
      </div>
      @endforeach
    </div>
